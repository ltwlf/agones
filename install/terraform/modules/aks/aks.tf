...
terraform {
  ...
  required_providers {
    azurerm = {
      ...
      version = ">= 3.94.0"  # Ensure this version or later is used
    }
  }
}
...
resource "azurerm_kubernetes_cluster" "agones" {
  ...
  # Don't change dns_prefix as node pool Network Security Group name uses a hash of dns_prefix on its name
  dns_prefix = "agones"
